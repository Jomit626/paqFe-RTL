...
class ProbGatter(implicit p: MixerParameter) extends Module {
  ...
  for(mixerIdx <- 0 until 8) {
    io.out(mixerIdx) <> Queue(DecoupledSimpleGatter(VecInit(io.in.map(_(mixerIdx))), mixerInputGatter)) // TODO: comb loop
    io.outCtx(mixerIdx) <> Queue(DecoupledSimpleGatter(VecInit(io.inCtx.map(_(mixerIdx)))))
  }
}
